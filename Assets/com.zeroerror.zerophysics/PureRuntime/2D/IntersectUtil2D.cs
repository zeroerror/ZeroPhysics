...
namespace ZeroPhysics.Physics2D {
    ...
    public static class IntersectUtil2D {
        ...
        public static bool HasCollision(Circle circle1, Circle circle2) {
            var center1 = circle1.Center;
            var center2 = circle2.Center;
            var xDiff = center1.x - center2.x;
            var yDiff = center1.y - center2.y;
            var radiusSum = circle1.Radius_scaled + circle2.Radius_scaled;
        ...
        public static bool HasCollision(Circle circle, Box2D rectangle) {
            ...
            if (circle.HasCollisionWithSphere(rectangle.C)) return true;
            if (circle.HasCollisionWithSphere(rectangle.D)) return true;

            var axisX = rectangle.GetAxisX();
            var axisX_PjSub1 = rectangle.GetAxisX_SelfProjectionSub();
            var axisX_PjSub2 = circle.GetProjectionSub(axisX);
            ...
            if (rectangle.Box2DType == Box2DType.AABB) {
                if (xOverlapCenter) {
                    ...
                    var axisY_PjSub2 = circle.GetProjectionSub(axisY);
                    var spherePjCenter_Y = (axisY_PjSub2.x + axisY_PjSub2.y) / 2;
                    bool yOverlapCenter = axisY_PjSub1.x < spherePjCenter_Y && spherePjCenter_Y < axisY_PjSub1.y;
                    return yOverlapCenter;
                }
            } else {
